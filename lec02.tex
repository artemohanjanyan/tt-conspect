...
$\beta$-редукция не обладает ромбовидным свойством (см. \ref{no-diamond}).
\begin{figure}[h]
    \centering
...
\begin{gather*}
    ...
    (\lambda x . x x) (\lambda x . x x x) \mathrel{\cancel{\rightrightarrows_{\beta}}} (\lambda x . x x x)(\lambda x . x x x)(\lambda x . x x x)
\end{gather*}
