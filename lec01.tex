...
\begin{example}
    \[
        (+1) \overline{1} =
        (\underline{\lambda n} . \lambda f . \lambda x . f (n f x)) \underline{(\lambda f . \lambda x . f x)} \rightarrow_{\beta}
        \lambda f . \lambda x . f ((\underline{\lambda f . \lambda x} . f x) \underline{f x}) \twoheadrightarrow_{\beta}
...
\begin{definition}[$\eta{}$-эквивалентность]
    ...
    \]
\end{definition}
Точно так же результат вычисления \mintinline{C++}{int f(int x)} равен результату вычисления \mintinline{C++}{[](int x){return f(x);}}.
...
\begin{proof}
    \begin{align*}
        \comb Yf &=_{\beta} (\lambda f . (\lambda x . f (x x)) (\lambda x . f (x x))) f \\
